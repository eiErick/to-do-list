...
if (savedTasksCompleted) {
	...
	tasksCompleted.forEach((tasksCompleted) => {
		const newTask = new Task(tasksCompleted.name);
...
function addTask() {
	...
	const task = capitalizeFirstLetter(taskInput.value);
...
document.addEventListener("click", (element) => {
	...
	if (clickedEditTaskName) {
		...
		btnComfirm.addEventListener("click", () => {
			...
			deleteTask(taskName, listName);
			const task = capitalizeFirstLetter(inputTextName.value);
			let itemExistente = false;
